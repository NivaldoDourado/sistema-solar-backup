...
export default function SetoresCusto() {
  ...
  const canCreateItem = canCreate("setorDeCusto");
  const canEditItem = canEdit("setorDeCusto");
  const canDeleteItem = canDelete("setorDeCusto");

  const { data: items, refetch } = trpc.setoresDeCusto.list.useQuery();
  const createMutation = trpc.setoresDeCusto.create.useMutation({
    onSuccess: () => {
  ...
  const updateMutation = trpc.setoresDeCusto.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = trpc.setoresDeCusto.delete.useMutation({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gerenciar plano de contas</p>
        </div>
        {canCreateItem && (
