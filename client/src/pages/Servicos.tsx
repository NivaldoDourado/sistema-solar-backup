...
export default function Servicos() {
  ...
  const canCreateItem = canCreate("servicos");
  const canEditItem = canEdit("servicos");
  const canDeleteItem = canDelete("servicos");

  const { data: items, refetch } = trpc.servicos.list.useQuery();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gerenciar tipos de serviços</p>
        </div>
        {canCreateItem && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <form onSubmit={handleSubmit}>
                <DialogHeader>
                  <DialogTitle>{editingId ? "Editar" : "Novo"} Serviços</DialogTitle>
                </DialogHeader>
