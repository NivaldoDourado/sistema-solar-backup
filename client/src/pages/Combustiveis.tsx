...
export default function Combustiveis() {
  ...
  const canCreateItem = canCreate("combustiveis");
  const canEditItem = canEdit("combustiveis");
  const canDeleteItem = canDelete("combustiveis");

  const { data: items, refetch } = trpc.combustiveis.list.useQuery();
  const createMutation = trpc.combustiveis.create.useMutation({
    onSuccess: () => {
  ...
  const updateMutation = trpc.combustiveis.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = trpc.combustiveis.delete.useMutation({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Combustiveis</h1>
          <p className="text-muted-foreground mt-1">Gerenciar combustiveis operacionais</p>
        </div>
        {canCreateItem && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <form onSubmit={handleSubmit}>
                <DialogHeader>
                  <DialogTitle>{editingId ? "Editar" : "Novo"} Combustiveis</DialogTitle>
                </DialogHeader>
