...
export default function GruposEquipamentos() {
  ...
  const canCreateItem = canCreate("gruposEquipamentos");
  const canEditItem = canEdit("gruposEquipamentos");
  const canDeleteItem = canDelete("gruposEquipamentos");

  const { data: items, refetch } = trpc.gruposDeEquipamentos.list.useQuery();
  const createMutation = trpc.gruposDeEquipamentos.create.useMutation({
    onSuccess: () => {
  ...
  const updateMutation = trpc.gruposDeEquipamentos.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = trpc.gruposDeEquipamentos.delete.useMutation({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Grupos de Equipamentos</h1>
          <p className="text-muted-foreground mt-1">Gerenciar grupos de equipamentos</p>
        </div>
        {canCreateItem && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <form onSubmit={handleSubmit}>
                <DialogHeader>
                  <DialogTitle>{editingId ? "Editar" : "Novo"} Grupo de Equipamentos</DialogTitle>
                </DialogHeader>
