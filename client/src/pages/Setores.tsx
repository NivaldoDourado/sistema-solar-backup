...
export default function Setores() {
  ...
  const canCreateItem = canCreate("setores");
  const canEditItem = canEdit("setores");
  const canDeleteItem = canDelete("setores");

  const { data: items, refetch } = trpc.setores.list.useQuery();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gerenciar setores operacionais</p>
        </div>
        {canCreateItem && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <form onSubmit={handleSubmit}>
                <DialogHeader>
                  <DialogTitle>{editingId ? "Editar" : "Novo"} Setores</DialogTitle>
                </DialogHeader>
